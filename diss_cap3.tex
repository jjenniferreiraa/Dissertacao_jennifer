...
O Método dos Pontos Materiais (MPM) é um método numérico desenvolvido para a análise de problemas envolvendo grandes deformações, contato e interação entre materiais. Sua formulação combina uma malha computacional fixa, responsável pela solução das equações de movimento, com um conjunto de pontos materiais que descrevem o comportamento do corpo ao longo da simulação. Esses pontos armazenam informações físicas e constitutivas do material, tais como massa, velocidade, tensões e deformações, sendo atualizados a cada passo de tempo conforme sua movimentação no domínio computacional. Diferentemente dos métodos tradicionais baseados exclusivamente em malha, o MPM reduz problemas associados à distorção excessiva dos elementos, tornando-se uma alternativa eficiente para a modelagem de fenômenos geotécnicos complexos \cite{yerro2022modelling,nguyen2023material}.

\begin{figure}[H]
	\centering
%	\includegraphics[width=0.6\linewidth]{cravacao/MPM2.pdf}
	\caption{Algoritmo de resolução do MPM. Fonte: Autores (2026)}
	\label{fig:MPM}
\end{figure}

A formulação do método baseia-se na conservação da quantidade de movimento, conforme apresentado na Eq. \eqref{eq:quantidade_movimento}. Sua integração numérica ocorre de forma incremental, iniciando-se pela avaliação das funções de forma nos pontos materiais e pela transferência das propriedades de massa e quantidade de movimento para os nós da malha de fundo (\ref{fig:MPM}a). Em seguida, as equações de equilíbrio são resolvidas na malha, considerando as contribuições das forças internas e externas atuantes (\ref{fig:MPM}b). Por fim, as velocidades e posições nodais atualizadas são novamente projetadas para os pontos materiais, que passam a representar o estado do sistema no novo instante de tempo (\ref{fig:MPM}c). Nessa formulação, $\sigma_p$ denota a tensão de Cauchy; $\nabla N_I(\mathbf{x}_p)$, o gradiente da função de forma; $V_p$ e $\rho_p$, respectivamente, o volume e a densidade do ponto material; $\mathbf{b}$, a força de corpo; e $\mathbf{f}_I^{int}$ e $\mathbf{f}_I^{ext}$, as forças nodais internas e externas. Ao término de cada passo de tempo, a malha é reinicializada, enquanto as partículas mantêm suas posições atualizadas, o que evita a distorção excessiva dos elementos \cite{martinelli2020numerical,yost2022,fu2024material}.
